...
Source: https://github.com/christopherjenness/DBCV/issues/13,
downloaded 5th of December 2020.

Modified to fit out own needs.
...
def _mutual_reach_dist_graph_multiproc(X, labels, dist_function):
    ...
    n_samples = np.shape(X)[0]
    point_members = [
        [((X[row], X[col], labels[row], labels[col])) for col in range(n_samples)]
        for row in range(n_samples)
    ]
    p = Pool(processes)
